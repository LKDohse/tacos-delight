...
public class ModModelProvider extends FabricModelProvider {
    ...
    public final void registerTallPlant(BlockStateModelGenerator generator, Block plant, Property<Integer> ageProperty) {
        ...
        ageProperty.getValues().forEach(value -> {
            Properties.DOUBLE_BLOCK_HALF.getValues().forEach(blockHalf -> {
                generator.createSubModel(plant, "_age" + value.toString() + "_" + blockHalf.name().toLowerCase(), Models.CROSS, TextureMap::cross);
            });
        });
